...
class AssignmentService {
  ...
  /// معرف إشعار موحّد وآمن (32-bit موجب دائماً).
  ///
  /// كانت الخدمة سابقاً تستخدم `id.hashCode` مباشرة بينما تستخدم الشاشات
  /// `id.hashCode & 0x7FFFFFFF`، مما يولّد فتحتَي إشعار مختلفتين لنفس التكليف
  /// (إشعار مكرر + إشعار يتيم لا يُلغى). توحيد المعرف هنا يجعل الجدولة
  /// (التي تُلغي ثم تُعيد الجدولة) عملية idempotent بلا تكرار.
  ...
  Future<Assignment> addAssignment({
    ...
    // ========== بداية التشخيص ==========
    print(
        '📝 [AssignmentService] addAssignment: title=$title, dueDate=$dueDate, hasReminder=$hasReminder');
    // ====================================
  ...
  Map<AssignmentStatus, int> getStatistics() {
    ...
    return {
      ...
      AssignmentStatus.inProgress:
          all.where((a) => a.status == AssignmentStatus.inProgress).length,
      AssignmentStatus.completed:
          all.where((a) => a.status == AssignmentStatus.completed).length,
