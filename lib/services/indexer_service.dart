...
class IndexerService {
  ...
  Future<void> indexFile(AssetModel asset) async {
    try {
      ...
      if (asset.type == 'photo') {
        thumb = await ThumbnailUtil.generateImageThumbnail(asset.resolvedFilePath, baseThumbDir);
      } else if (asset.type == 'video') {
        thumb = await ThumbnailUtil.generateVideoThumbnail(asset.resolvedFilePath, baseThumbDir);
      } 
  ...
  Future<void> _indexPdfWithLimit(AssetModel asset) async {
    try {
      ...
      final pdfInfo = await getPdfInfo(asset.resolvedFilePath);
      ...
      if (pdfInfo.isLargeFile) {
        ...
        extractedText = await compute(extractLimitedPdfCompute, asset.resolvedFilePath);
      } else {
        // للملفات الصغيرة، استخدم الاستخراج العادي
        extractedText = await compute(extractTextFromPdfCompute, asset.resolvedFilePath);
      }
      ...
      try {
        final extractedText = await compute(extractFirstFewPagesCompute, asset.resolvedFilePath);
        if (extractedText != null && extractedText.isNotEmpty) {
          await metadata.updateExtractedText(asset.id, extractedText);
