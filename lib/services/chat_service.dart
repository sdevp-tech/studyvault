...
class ChatService {
  ...
  Future<void> sendMessage(String chatId, String text, {String? replyToId, String? replyToText}) async {
    ...
    // 1) الكتابة المحلية التفاؤلية فوراً بحالة "قيد الإرسال" (status 0)
    final localMsg = LocalMessage(
      ...
      text: text,
      timestamp: now,
      status: 0, // 0 = pending/sending until the server confirms
    ...
    // 2) محاولة الرفع للخادم — في حال الفشل (أوفلاين) تبقى الرسالة status=0
    // وتُعاد محاولة إرسالها عند استعادة الاتصال.
    await _pushMessageToServer(chatId, localMsg);
  }

  /// يحاول دفع رسالة محلية إلى Firestore ويحدّث حالتها إلى 1 عند النجاح.
  Future<void> _pushMessageToServer(String chatId, LocalMessage msg) async {
    final myUid = _auth.currentUser?.uid;
    if (myUid == null) return;
    ...
    try {
      await messageRef.set({
        ...
        'status': 1,
        'expireAt': Timestamp.fromDate(msg.timestamp.add(const Duration(hours: 48))),
      ...
      // تأكيد الإرسال محلياً
      final stored = _messageBox.get(msg.messageId);
      if (stored != null && stored.status < 1) {
        stored.status = 1;
        await stored.save();
      }
    } catch (e) {
      // تبقى status=0 (قيد الإرسال) لإعادة المحاولة لاحقاً
      print('⚠️ فشل إرسال الرسالة (ستعاد المحاولة عند الاتصال): $e');
    }
  }

  /// يعيد إرسال كل رسائل المستخدم المعلّقة (status=0). يُستدعى عند استعادة الاتصال.
  Future<void> resendPendingMessages([String? chatId]) async {
    final myUid = _auth.currentUser?.uid;
    if (myUid == null) return;

    final pending = _messageBox.values
        .where((m) =>
            m.senderId == myUid &&
            m.status == 0 &&
            !m.isDeleted &&
            (chatId == null || m.chatId == chatId))
        .toList();

    for (final msg in pending) {
      await _pushMessageToServer(msg.chatId, msg);
    }
  }
