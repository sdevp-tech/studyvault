...
class TodoService {
  ...
  Future<TodoItem> addTodo({
    ...
    // ========== بداية التشخيص ==========
    print('📝 [TodoService] addTodo: title=$title, dueDate=$dueDate, hasReminder=$hasReminder');
    // ====================================
    ...
    if (hasReminder && dueDate != null && dueDate.isAfter(DateTime.now())) {
      ...
      print('   - لن يتم جدولة إشعار (hasReminder=$hasReminder أو dueDate=null أو الوقت في الماضي)');
    }
  ...
  Future<void> updateTodo(TodoItem todo) async {
    ...
    if (todo.hasReminder && todo.dueDate != null && todo.dueDate!.isAfter(DateTime.now()) && !todo.isCompleted) {
      try {
        await NotificationService().scheduleNotification(
          id: todo.id.hashCode,
          title: 'مهمة مستحقة',
  ...
  Map<Priority, int> getPriorityStatistics() {
    ...
    return {
      Priority.urgent: activeTodos.where((t) => t.priority == Priority.urgent).length,
      Priority.high: activeTodos.where((t) => t.priority == Priority.high).length,
      Priority.medium: activeTodos.where((t) => t.priority == Priority.medium).length,
      Priority.low: activeTodos.where((t) => t.priority == Priority.low).length,
    };
  }
